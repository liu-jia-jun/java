...
//链表的反转
	...
	/*		
	 * 
	 * 这里将temp赋给temp2 是地址传递  之后的操作都会影响原来的单链表  
	 * 之后链表就会只有一个有效节点和一个头节点
	 * 所以说这个操作是错误的
       			temp2=temp;
...
class Test {
	...
	/*
	 * 
	 * 输入有问题 public static void test() { boolean state = true; int n; int number;
	 * String name = null; int great = 0; String choose;
	 * 
	 * 
	 * Link link = new Link(); Scanner sc = new Scanner(System.in);
	 * System.out.println("输入字母  表示你接下来的操作"); System.out.println(
	 * "a为顺序添加元素\ns为输出展示所以元素\nd为删除指定元素\nc为查询指定元素\nch为改变元素内容\nab为按序添加\ng为返回元素个数e为退出程序"
	 * ); while (state) { System.out.println("请输入字母操作，回车表示输入完成");
	 * 
	 * choose = sc.nextLine();
	 * 
	 * switch (choose) { case "a": System.out.println("请输入序号，大于0的正整数"); number =
	 * sc.nextInt();
	 * 
	 * 
	 * System.out.println("请输入成绩"); if(sc.hasNextInt()) great = sc.nextInt();
	 * 
	 * System.out.println("请输入姓名"); if(sc.hasNext()) name = sc.nextLine();
	 * 
	 * link.add(name, number, great); System.out.println("添加成功"); break; case "s":
	 * link.show(); break; case "d": System.out.println("请输入你要删除数据的序号"); n =
	 * sc.nextInt();
	 * 
	 * link.deleat(n); break; case "c": System.out.println("请输入你要查询数据的序号"); n =
	 * sc.nextInt();
	 * 
	 * link.check(n); break; case "ch": System.out.println("请输入你要修改的数据的序号"); n =
	 * sc.nextInt();
	 * 
	 * System.out.println("请输入序号，大于0的正整数"); number = sc.nextInt();
	 * 
	 * System.out.println("请输入姓名"); name = sc.nextLine();
	 * 
	 * System.out.println("请输入成绩"); great = sc.nextInt();
	 * 
	 ...
	 * System.out.println("请输入成绩"); great = sc.nextInt();
	 * 
	 * link.addBy(name, number, great); System.out.println("添加成功"); break; case "g":
	 * System.out.println(link.getlength()); break; case "e": state = false; break;
	 * default: System.out.println("无效操作"); break; }
	 * 
